...
//            console.log(this.formData.icon)
        ...
        if (typeof this.formData.news_time === 'number') {
          newsTime = parseInt(this.formData.news_time / 1000)
        } else {
          newsTime = parseInt(new Date(this.formData.news_time).getTime() / 1000)
        }
...
//        }
      ...
      async handleEdit (index, row) {
        ...
        if (res.param.list.template === 3) {
          this.content = JSON.parse(res.param.list.content)
        } else {
