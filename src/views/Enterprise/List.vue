  ...
  <section>
    ...
    <el-table :data="list"
      ...
      <el-table-column label="操作" width="280" fixed="right">
        <template slot-scope="scope">
          <el-button size="small" @click="handlePost(scope.$index, scope.row)">岗位设置</el-button>
...
<script>
  ...
  const MODEL_NAME = 'Enterprise' // API模块名
  ...
  export default {
    ...
    methods: {
      ...
      handlePost (index, row) {
        console.log(this.$route.path)
        this.$router.push(`${this.$route.path}/ejob/${row.id}`)
      },
